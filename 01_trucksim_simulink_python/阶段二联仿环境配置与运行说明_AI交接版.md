...
已完成本机迁移适配：MATLAB 入口脚本会按自身所在目录自动推导阶段二根目录，并自动优先检测本机 Python 与 TruckSim 路径。若目标机路径再次变化，可用环境变量覆盖：

- `P2_PYTHON_EXE`
- `P2_TRUCKSIM_SIMFILE`
- `P2_TRUCKSIM_SOLVER_DIR`

| 项目 | 当前路径 |
|---|---|
| 阶段二根目录 | `C:\动力学仿真环境阶段二\01_trucksim_simulink_python` |
| MATLAB 一键脚本 | `C:\动力学仿真环境阶段二\01_trucksim_simulink_python\01_一键启动脚本` |
| Simulink 模型目录 | `C:\动力学仿真环境阶段二\01_trucksim_simulink_python\00_simulink` |
| Python 控制器目录 | `C:\动力学仿真环境阶段二\01_trucksim_simulink_python\05_python_controller` |
| TXT 测试用例目录 | `C:\动力学仿真环境阶段二\01_trucksim_simulink_python\02_测试用例` |
| 单工况数据输出 | `C:\动力学仿真环境阶段二\01_trucksim_simulink_python\03_数据存储\Python联仿` |
| 测试报告输出 | `C:\动力学仿真环境阶段二\01_trucksim_simulink_python\04_测试报告` |
...
- TruckSim 2019.0。
- 当前本机 Python 解释器为 `C:\Python\python\python3.10.4\python.exe`。
...
```powershell
cd "C:\动力学仿真环境阶段二\01_trucksim_simulink_python\05_python_controller"
...
```matlab
cd('C:\动力学仿真环境阶段二\01_trucksim_simulink_python\01_一键启动脚本')
run_case_python
```
...
3. COM 不可用时，启用方法2：
   ...
   - 写入 `TSTOP`、`SPEED`、`SPEED_TARGET_CONSTANT`、`MY_FRICTION` 和速度控制器增益
   - 仅当 `trucksim.force_internal_powertrain_for_speed_control: true` 时，才额外写入动力链内部模式开关
...
- 如果初始速度正确但仿真中持续掉速，例如 40 km/h 掉到约 31 km/h，优先在 TruckSim GUI 工况中确认发动机、液力变矩器/动力耦合、变速箱、换挡命令和驱动桥差速器均为内部模式。当前工况已在 GUI 中修正，`vehicle_config.yaml` 默认保持 `trucksim.force_internal_powertrain_for_speed_control: false`，避免脚本覆盖 GUI 选择；只有迁移旧工况且无法手动修正时，才临时改为 `true` 并检查日志是否写入 `OPT_ENGINE_INTERNAL`、`OPT_PWR_CPL_INTERNAL`、`OPT_TRANS_INTERNAL`、`OPT_SHIFT_INTERNAL`、`OPT_DIFF_INTERNAL(2)`。
- 如果速度能回到目标但波动偏大，检查 `vehicle_config.yaml: trucksim.speed_controller`。当前推荐值为 `kp=0.35`、`ki=0.06`、`kp3=0`，用于替代 TruckSim 默认的 `SPEED_KP=0.14`、`SPEED_KI=0.16`，可减小慢速积分振荡。
