...
def metrics(rows, case):
    ...
    m = {
        ...
        "v_min_kmh": float(np.nanmin(v)) if len(v) else np.nan,
        "v_max_kmh": float(np.nanmax(v)) if len(v) else np.nan,
        "v_peak_to_peak_kmh": float(np.nanmax(v) - np.nanmin(v)) if len(v) else np.nan,
        "v_std_kmh": float(np.nanstd(v)) if len(v) else np.nan,
...
def write_metrics_csv(run_dir, run_no, m):
    ...
    with open(out, "w", newline="", encoding="utf-8") as f:
        ...
        w.writerow(["v_min", "%.15g" % m["v_min_kmh"], "km/h"])
        w.writerow(["v_max", "%.15g" % m["v_max_kmh"], "km/h"])
        w.writerow(["v_peak_to_peak", "%.15g" % m["v_peak_to_peak_kmh"], "km/h"])
        w.writerow(["v_std", "%.15g" % m["v_std_kmh"], "km/h"])
...
def add_case_content(doc, run_no, case, m, report_dir):
    ...
    params = [
        ...
        ("车速波动（最小 / 最大 / 峰峰值）", "%.4g / %.4g / %.4g km/h" % (
            m["v_min_kmh"], m["v_max_kmh"], m["v_peak_to_peak_kmh"])),
...
def batch_summary(results, report_root):
    ...
    with open(summary_csv, "w", newline="", encoding="utf-8") as f:
        ...
        w.writerow(["run_number", "case_name", "duration_s", "v_start_kmh", "v_end_kmh",
                    "v_min_kmh", "v_max_kmh", "v_peak_to_peak_kmh", "v_std_kmh",
        ...
        for run_no, case_name, m in results:
            w.writerow([run_no, case_name,
                        ...
                        "%.4g" % m["v_end_kmh"], "%.4g" % m["v_min_kmh"],
                        "%.4g" % m["v_max_kmh"], "%.4g" % m["v_peak_to_peak_kmh"],
                        "%.4g" % m["v_std_kmh"], "%.4g" % m["max_abs_beta_deg"],
