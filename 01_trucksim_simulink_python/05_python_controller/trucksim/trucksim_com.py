...
PAR_KEYWORDS = {
    ...
    "road_friction_scale": "MY_FRICTION",
    "engine_internal": "OPT_ENGINE_INTERNAL",
    "power_coupling_internal": "OPT_PWR_CPL_INTERNAL",
    "transmission_internal": "OPT_TRANS_INTERNAL",
    "shift_internal": "OPT_SHIFT_INTERNAL",
    "differential_internal": "OPT_DIFF_INTERNAL(2)",
    "speed_kp": "SPEED_KP",
    "speed_ki": "SPEED_KI",
    "speed_kp3": "SPEED_KP3",
    "speed_integral_deadband": "VX_IERR_DEAD_SC",
}
INSERT_AFTER_SPEED_CONTROLLER = {
    "SPEED_KP",
    "SPEED_KI",
    "SPEED_KP3",
    "VX_IERR_DEAD_SC",
...
def _patch_keyword_lines(text, keyword, value):
    pattern = re.compile(r"^(\s*\*?%s(?:\s*=\s*|\s+))([-+0-9.eE]+)(\s*(?:[#!;].*)?)$" % re.escape(keyword),
                         re.MULTILINE)
...
def _insert_keyword_after(text, anchor_keyword, keyword, value):
    line = "%s %.15g" % (keyword, value)
    pattern = re.compile(r"^(\s*%s\s*(?:[#!;].*)?\r?\n)" % re.escape(anchor_keyword),
                         re.MULTILINE)
    text2, count = pattern.subn(r"\g<1>%s\n" % line, text, count=1)
    if count:
        return text2, 1
    return text.rstrip() + "\n" + line + "\n", 1
...
def _patch_one_par_file(path, updates, required_keywords=None, insert_missing_keywords=None):
    try:
    ...
    for keyword, value in updates.items():
        ...
        if count == 0 and keyword in (insert_missing_keywords or ()):
            text, count = _insert_keyword_after(text, "INSTALL_SPEED_CONTROLLER", keyword, value)
...
def _source_updates_for(path, data_root, all_updates):
    ...
    elif rel_lower.startswith("powertrain\\"):
        keys = (
            "OPT_ENGINE_INTERNAL",
            "OPT_PWR_CPL_INTERNAL",
            "OPT_TRANS_INTERNAL",
            "OPT_SHIFT_INTERNAL",
            "OPT_DIFF_INTERNAL(2)",
        )
...
def _patch_run_all_par(case, cfg):
    ...
    if trucksim.get("force_internal_powertrain_for_speed_control", True):
        updates.update({
            PAR_KEYWORDS["engine_internal"]: 1.0,
            PAR_KEYWORDS["power_coupling_internal"]: 1.0,
            PAR_KEYWORDS["transmission_internal"]: 1.0,
            PAR_KEYWORDS["shift_internal"]: 1.0,
            PAR_KEYWORDS["differential_internal"]: 1.0,
        })
    speed_controller = trucksim.get("speed_controller", {}) or {}
    speed_controller_map = {
        "kp": "speed_kp",
        "ki": "speed_ki",
        "kp3": "speed_kp3",
        "integral_deadband_m": "speed_integral_deadband",
    }
    for cfg_key, par_key in speed_controller_map.items():
        if cfg_key in speed_controller and speed_controller[cfg_key] is not None:
            updates[PAR_KEYWORDS[par_key]] = float(speed_controller[cfg_key])
    ...
    for i, path in enumerate(patch_files):
        ...
        if not file_updates:
            continue
        insert_missing = INSERT_AFTER_SPEED_CONTROLLER if i == 0 else ()
        counts, _missing, backup_path, error = _patch_one_par_file(
            path, file_updates, insert_missing_keywords=insert_missing)
        if error:
