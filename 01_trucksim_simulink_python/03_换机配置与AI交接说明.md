...
本机 TCP 固定使用 `127.0.0.1:50007`。Python 进程会尝试根据 TXT 修改 TruckSim 工况；若 COM 不可用，默认可回退到 `simfile.sim` 指向的 `Run_all.par` 及其关联 `.par` 文件。
...
本机验证环境为 Windows、MATLAB R2021a、TruckSim 2019.0、Python 3.10.4（`C:\Python\python\python3.10.4\python.exe`）。Python 需要先安装以下库： `pyyaml`、`numpy`、`matplotlib`、`python-docx`；`pywin32` 仅用于 TruckSim COM，非 COM 回退路径不依赖它。
...
8. 比较 `*_trucksim_io.csv` 与 `*_python_signals.csv` 的 `sim_time_s` 末值；两者均达到 TXT 停止时间才算时间轴验收通过。
