...
确认 TruckSim Run、备份和写入许可后，再执行 `run_case_python`。单工况完整通过后才可执行 `run_batch_python`。阶段二会修改当前 TruckSim `.par` 数据，因此不得与阶段一或另一套阶段二实例并行运行。

## 当前数据边界

第032次的 Python TCP 时间覆盖 0 至 20 s，但同次 `trucksim_io.csv` 只到 2 s；时间向量与状态信号行数仍未完全对齐。因此当前报告可用于查看通讯与控制响应，但不能作为原始 TruckSim 时间轴已修复的证据。下一次真实运行必须确认 `trucksim_io.csv` 与 `python_signals.csv` 的 `sim_time_s` 末值均等于 TXT 的 `stop_time_s`。
