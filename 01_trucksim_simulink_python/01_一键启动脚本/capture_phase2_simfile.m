function targetSimfile = capture_phase2_simfile(sourceSimfile)
% CAPTURE_PHASE2_SIMFILE 保存阶段二Run的专用Solver入口。
% 先在TruckSim中选中阶段二Run并“发送到Simulink”，再运行本函数。
% 仅复制simfile入口，不会修改TruckSim的Run或任何.par文件。

scriptDir = fileparts(mfilename('fullpath'));
projectRoot = fileparts(scriptDir);
runtimeDir = fullfile(projectRoot, 'runtime');
pointerFile = fullfile(runtimeDir, 'trucksim_phase2.path');
...
assert(exist(sourceSimfile, 'file') == 2, ...
    'capture_phase2_simfile:MissingSource', '发送到Simulink后生成的simfile不存在: %s', sourceSimfile);
sourceInput = resolve_input(sourceSimfile);
assert(exist(sourceInput, 'file') == 2, ...
    'capture_phase2_simfile:InvalidInput', '源simfile的INPUT无法定位参数文件: %s', sourceInput);

% 若同级阶段一已经保存入口，禁止两个阶段指向同一Run。
otherPointer = fullfile(fileparts(projectRoot), '00trucksim_simulink', ...
...
if exist(otherPointer, 'file') == 2
    ...
    if exist(otherSimfile, 'file') == 2
        ...
        assert(~strcmpi(sourceInput, otherInput), ...
            'capture_phase2_simfile:SharedRun', ...
            '阶段二和阶段一不能指向同一Run_all.par。请在TruckSim选择阶段二专用Run后重新发送到Simulink。');
    else
        warning('capture_phase2_simfile:OtherStageNotConfigured', ...
            '阶段一入口仍是旧电脑路径或尚未配置：%s；本次仅跳过阶段一共享Run检查。', otherSimfile);
    end
end

if exist(runtimeDir, 'dir') ~= 7, mkdir(runtimeDir); end
% TruckSim S-Function只接受数据目录中的simfile，因此副本与源文件同目录保存。
targetSimfile = fullfile(fileparts(sourceSimfile), 'simfile_phase2.sim');
if exist(targetSimfile, 'file') == 2
    backup = [targetSimfile '.previous_' datestr(now, 'yyyymmdd_HHMMSS')];
    copyfile(targetSimfile, backup, 'f');
    fprintf('保留旧阶段二simfile: %s\n', backup);
end
copyfile(sourceSimfile, targetSimfile, 'f');

metaFile = fullfile(runtimeDir, 'trucksim_phase2.origin.txt');
fid = fopen(metaFile, 'w', 'n', 'UTF-8');
assert(fid >= 0, 'capture_phase2_simfile:MetaWrite', '无法写入: %s', metaFile);
...
assert(fid >= 0, 'capture_phase2_simfile:PointerWrite', '无法写入: %s', pointerFile);
cleanupPointer = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, '%s\n', targetSimfile);
clear cleanupPointer
fprintf('阶段二专用simfile已保存: %s\n', targetSimfile);
fprintf('项目入口记录已保存: %s\n', pointerFile);
fprintf('其INPUT指向: %s\n', sourceInput);
end
...
assert(~isempty(token), 'capture_phase2_simfile:NoInput', 'simfile中没有INPUT行: %s', simfile);
expr = token{1};
