...
useTrucksimCom = getopt(opt, 'useTrucksimCom', false);
...
cmd = sprintf('start /b "" "%s" tcp_server.py --case "%s" --port %d --log-dir "%s" --config-trucksim %d > "%s" 2>&1', ...
    pythonExe, asciiCase, port, logDir, double(useTrucksimCom), logFile);
[st, ~] = system(cmd);
cd(oldDir);
if st ~= 0
...
for k = 1:180
    ...
    if exist(logFile, 'file')
        ...
        if contains(txt, 'CONFIG_FAIL')
            error('run_single_python_case:ComConfigFailed', ...
        ...
        elseif contains(txt, 'SERVER_ERROR') || contains(txt, 'Traceback')
    ...
    if abs(v0_actual - v0_case) > 0.5
        warning('run_single_python_case:SpeedMismatch', ...
            ['TruckSim 实际初始车速 %.2f km/h 与用例设定 %.2f km/h 不一致。' ...
             '如需按用例设置 TruckSim 工况，请先验证 trucksim_com.py 的 COM 映射并开启 useTrucksimCom。'], ...
            v0_actual, v0_case);
    end
catch
end
