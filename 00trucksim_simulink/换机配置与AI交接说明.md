...
本文件对应 `00trucksim_simulink`，即阶段一的 TruckSim + Simulink 一键仿真环境。把本文件、同级阶段二文件夹和该文件夹一并提供给 AI，可用于在另一台 Windows 电脑上检查和适配环境。
...
当前入口已具备配置与导出前验收：`trucksim_config.m` 调用 `configure_trucksim_case.py`，优先使用 TruckSim COM；COM 不可用时，使用同级阶段二的配置模块更新当前 `simfile.sim` 所关联的 `.par` 文件。仿真后 `validate_simulation_output.m` 核对实际结束时间、初始车速和第一轴转向。配置或验收失败时不会生成新的 CSV/报告。
...
本项目历史验证环境是 MATLAB R2021a、TruckSim 2019.0、Python。Python 报告程序需要 `numpy`、`matplotlib`、`python-docx`。当前 `trucksim_config.m` 优先尝试 TruckSim COM；COM 不可用时复用阶段二的 `.par` 文件回退配置，因此 COM 不是运行前提。

阶段一的回退配置代码当前从同级 `01_trucksim_simulink_python/05_python_controller/trucksim/trucksim_com.py` 导入。因此，若只复制阶段一文件夹，严格配置会缺少该模块；换机时应保持两个阶段文件夹同级，或由维护者先将该共享模块改为阶段一自包含实现并完成验证。

## AI 的换机处理顺序

1. 只读检查项目文件是否完整，确认模型、入口脚本、用例和报告脚本存在。
2. 检查目标机给出的 Python、`simfile.sim`、solver 目录是否真实存在，并确认 solver 目录有 `Matlab84+` 子目录。
3. 在修改前列出旧路径、新路径和将修改的文件；只修改下方列出的源脚本，不修改历史 CSV、报告或 manifest 中的旧路径。不得同时运行阶段一和阶段二，因为两者会改动同一 TruckSim 工况参数文件。
...
`trucksim_config.m` 默认启用严格配置。COM 不可用时，它会通过当前 `simfile.sim` 定位并更新 `Run_all.par` 及关联工况文件；首次修改会保留备份。若配置或后续输出验收失败，脚本会停止且不导出 CSV/报告。该修改发生在 TruckSim 项目数据目录，属于共享外部状态；在测试结束前不要用阶段二或 TruckSim 图形界面改写同一套 Run。
...
| 阶段一运行后阶段二工况异常 | 两项目共享外部 TruckSim `.par` 状态；恢复确认过的 Run/备份后，再单独运行其中一个项目 |
...
不得修改历史结果；严格配置已实现，但只有在我明确允许改动 TruckSim 数据目录时才可执行；不得并行运行阶段一和阶段二。
